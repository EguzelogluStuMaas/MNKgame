...
public class BigToeBoard {
    ...
    public GameStatus selectCell(int i, int j) throws IndexOutOfBoundsException, IllegalStateException {
        ...
        {
            gameState = (currentPlayer == 0) ? GameStatus.WIN_PLAYER_1 : GameStatus.WIN_PLAYER_2;
        }
...
/*Same check but columnar check */
   ...
   //Diagonal check
    ...
    if(count>=K) return true;
    //Diagonal backwards check EMPTY)
    count=1;
    for(int k=1;k>=0 && BOARD[i-k][j+k]==currentState;k++)
    {
        count++;
